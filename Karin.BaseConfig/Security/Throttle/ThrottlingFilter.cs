...
namespace Karin.BaseConfig.Security.Throttle
{
    public class ThrottlingFilter : ActionFilterAttribute, IActionFilter
    {
            ...
            if (Policy != null && applyThrottling)
            {
                ...
                if (!IsWhitelisted(identity))
                {
                    ...
                    foreach (var rate in rates)
                    {
                        ...
                        if (throttleCounter.Timestamp + timeSpan < System.DateTime.UtcNow)
                            continue;
        ...
        private string RetryAfterFrom(System.DateTime timestamp, RateLimitPeriod period)
        {
            var secondsPast = Convert.ToInt32((System.DateTime.UtcNow - timestamp).TotalSeconds);
            var retryAfter = 1;
            switch (period)
            {
                case RateLimitPeriod.Minute:
